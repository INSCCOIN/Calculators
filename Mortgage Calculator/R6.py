...
def calculate_mortgage():
    ...
    interest_paid = [entry[2] for entry in amortization_schedule]
    principal_paid = [entry[3] for entry in amortization_schedule]
    ...
    ax1.plot(months, balances, label='Remaining Balance', color=color)
    ax1.tick_params(axis='y', labelcolor=color)

    # Highlight each month's payment
    for month in range(1, num_payments + 1):
        ax1.axvline(x=month, color='gray', linestyle='--', alpha=0.5)

    # Plot markers for important points (loan payoff)
    ax1.axhline(y=0, color='red', linestyle='--', label='Loan Payoff', alpha=0.7)

    ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis

    # Plot interest and principal payments
    color = 'tab:orange'
    ax2.set_ylabel('Payment Amount', color=color)
    ax2.plot(months, interest_paid, label='Interest Paid', color=color, linestyle='dotted')
    ax2.plot(months, principal_paid, label='Principal Paid', color=color, linestyle='dashed')
    ax2.tick_params(axis='y', labelcolor=color)

    fig.tight_layout()  # ensure proper spacing of labels and plots
